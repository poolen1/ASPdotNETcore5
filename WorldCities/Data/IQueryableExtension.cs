...
namespace WorldCities.Data
{
    public static class IQueryableExtension
    {
        public static (string, IEnumerable<SqlParameter>) ToParametrizedSql(this IQueryable query)
        {
            ...
            string selectExpressionsText = "_selectExpression";
            string querySqlgeneratorFactoryText = "+querySqlGeneratorFactory";
...
#pragma warning restore EF1001 // Internal EF Core API usage.
                ...
                parameters = parameterValues.Where(parameterValues => 
                    parameterNames.Contains(parameterValues.Key)).Select(pv => new SqlParameter("@" + pv.Key, pv.Value)).ToList();
            }
            else
            {
                SelectExpression selectExpression = enumerator.Private<SelectExpression>(selectExpressionsText) 
                    ?? throw new InvalidOperationException($"{cannotGetText} {selectExpressionsText}");
        ...
        private static T Private<T>(this object obj, string privateField) => (T)
            obj?.GetType().GetField(privateField, bindingFlags)?.GetValue(obj);
    }
}
